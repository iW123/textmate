...
static void DrawText (std::string const& text, CGRect const& rect, CGFloat baseline, NSFont* font, NSColor* color)
{
    ...
    NSLog(@"CTLine bounds=%@", NSStringFromRect(CGRectMake(CGRectGetMaxX(rect) - CTLineGetTypographicBounds(line, NULL, NULL, NULL), baseline - 12, CTLineGetTypographicBounds(line, NULL, NULL, NULL), 20)));
...
{
    NSLog(@"WSdrawRect: %@", NSStringFromRect(aRect));
    [self.backgroundColor set];
    NSRectFill(NSIntersectionRect(aRect, self.frame));
    [[NSColor blueColor] set];
    NSRectFill(NSMakeRect(14, 10, 10, 10));
    ...
    for(CGFloat y = NSMinY(aRect); y < NSMaxY(aRect); )
    {
        ...
        for(auto const& dataSource : [self visibleColumnDataSources])
        {
            ...
            if(dataSource.identifier == GVLineNumbersColumnIdentifier.UTF8String)
            {
                ...
                NSLog(@"draw line number=%lu color=%@",
                      (unsigned long)record.lineNumber + 1,
                      textColor);
